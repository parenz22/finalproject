...
def simulate_bungee_excel(m, L, E, d, b, dt):
    """
    Matches the Excel Differential Equation Solution.
    m: mass (kg), L: natural length (m), E: Young's Modulus (Pa)
    d: diameter (m), b: damping coeff, dt: time step (s)
    """
    A = np.pi * (d / 2)**2
    k = (E * A) / L
    
    # State variables (following Excel coordinates: y=0 is top, positive is down)
    y = 0.0
    v = 0.0
    a = G_ACCEL # Starting acceleration
    t = 0.0
    
    sim_time = 40.0
    times, positions, velocities, accels, gs = [], [], [], [], []
    ...
    while t < sim_time:
        ...
        # Calculate Felt G: (Spring Force + Damping Force) / (m * g)
        # This represents the force pushing back against the jumper's weight
        stretch = max(0.0, y - L)
        f_spring = k * stretch
        f_damping = b * v
        felt_g = (f_spring + f_damping) / (m * G_ACCEL)
        gs.append(felt_g)

        # 1. Update Position using current Velocity (Excel: y_new = y_old + v_old * dt)
        y = y + v * dt
        
        # 2. Update Velocity using current Acceleration (Excel: v_new = v_old + a_old * dt)
        v = v + a * dt
        
        # 3. Update Acceleration for the next step based on new position/velocity
        # Excel Formula: Net Force = mg - Fs - Fd
        new_stretch = max(0.0, y - L)
        f_s = k * new_stretch
        f_d = b * v
        f_net = (m * G_ACCEL) - f_s - f_d
        a = f_net / m
    ...
    return {
        ...
        "heights": DROP_H - np.array(positions), # Convert displacement to height
...
with col_input:
    ...
    b = st.slider("**Damping Coefficient (b)**", 0, 20, 5)
    dt = st.selectbox("**Time Step (dt)**", [0.01, 0.001], index=0)

    sim = simulate_bungee_excel(m, L, E, d, b, dt)
